s = '''8C TS KC 9H 4S 7D 2S 5D 3S AC
...
def val(hand):
	values = [hand[i][0] for i in range(5)]
	for i,v in enumerate(values[:]):
		if v in list('23456789'):
...
def poker(hand):
	...
	if flush(hand) and set([hand[i][0] for i in range(5)]) == set(list('TJQKA')): #Royal Flush
		#print('RF')
		return 9
	elif flush(hand) and straight(hand): #Straight Flush
		#print('SF')
		return 8
	elif any(4==values.count(v) for v in set(values)): #Four of a Kind
		#print('4K')
		return 7
	elif len(set(values)) == 2: #Full House
		#print('FH')
		return 6
		...
		#print('S')
		return 4
	elif any(3==values.count(v) for v in set(values)): #Three of a Kind
		#print('3K')
		return 3
	elif len(set(values)) == 3: #Two Pairs
		#print('2P')
		return 2
	elif any(2==values.count(v) for v in set(values)): #One Pair
		#print('1P')
		return 1
	else: #High Card
		return 0
...
def tiebreak_p1(h1,h2,score):
	values1 = val(h1)
	values2 = val(h2)

	if score in [8,5,4,0]:
		v1,v2 = values1,values2
		for i in range(1,len(v1)+1): #12345
			if v1[-i] > v2[-i]:
				return True
			elif v1[-i] < v2[-i]:
				return False
		...
		v1 = [v for v in set(values1) if values1.count(v)==4][0]
		v2 = [v for v in set(values2) if values2.count(v)==4][0]
		if v1 > v2:
			return True
		elif v2 < v1:
			return False
	elif score == 6:
		v1 = [v for v in set(values1) if values1.count(v)==2] + [v for v in set(values1) if values1.count(v)==3]
		v2 = [v for v in set(values2) if values2.count(v)==2] + [v for v in set(values2) if values2.count(v)==3]
		...
		v1 = [v for v in set(values1) if values1.count(v)==3][0]
		v2 = [v for v in set(values2) if values2.count(v)==3][0]
		if v1 > v2:
			return True
		elif v2 < v1:
			return False
	elif score in [2,1]:
		v1 = sorted([v for v in set(values1) if values1.count(v)==2])
		v2 = sorted([v for v in set(values2) if values2.count(v)==2])
		for i in range(1,len(v1)+1):
			if v1[-i] > v2[-i]:
				return True
			elif v1[-i] < v2[-i]:
				...
				continue
		v1_1 = sorted([v for v in set(values1) if values1.count(v)==1])
		v2_1 = sorted([v for v in set(values2) if values2.count(v)==1])
		for i in range(1,len(v1_1)+1):
			if v1_1[-i] > v2_1[-i]:
				return True
			elif v1_1[-i] < v2_1[-i]:
				return False
...
for g in games:
	print(g)
	p1_hand,p2_hand = g[0],g[1]
	p1_score,p2_score = poker(g[0]), poker(g[1])
	...
	if p1_score > p2_score:
		...
		if tiebreak_p1(p1_hand,p2_hand,p1_score):
			p1_win += 1
